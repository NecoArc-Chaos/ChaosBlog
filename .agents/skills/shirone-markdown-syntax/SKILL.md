...
| 语法 | 写法 | 演示文章 |
|---|---|---|
| Field cards / 字段卡片 | `:::: field-group` + `::: field name` with `@type`, `@default`, and required-state metadata | `[demo removed]` |
| 提示容器 | `:::tip[标题] ... :::` 或 `> [!NOTE]`,类型:`note/info/tip/important/warning/caution/details` | `[demo removed]` |
| 折叠面板 | `::: collapse [accordion] [expand]` 包裹无序列表,项首 `:+`/`:-` 控制开合 | `[demo removed]` |
| 选项组(Tabs) | `::: tabs[#同步id]` + `@tab 标题#值`,值相同的组跨页同步 | `[demo removed]` |
| 步骤流 | `:::steps{title start}` 包裹有序列表 | `[demo removed]` |
| 代码树 | `:::code-tree{title height entry icon}` 内嵌多文件代码块,或 `@[code-tree](目录)` | `[demo removed]` |
| 文件树 | `:::file-tree{title icon}` 嵌套列表或 ```file-tree 围栏(tree 输出) | `[demo removed]` |
| 马克笔高亮 | `==内容==`,变体 `==...=={.error}`(`primary/secondary/tertiary/error/tip`) | `[demo removed]` |
| 数学公式 | `$行内$` 与 `$$块级$$`(KaTeX) | `[demo removed]` |
| Mermaid 图 | ` ```mermaid ` 围栏,客户端按需主题化渲染 | `[demo removed]` |
| 图片画廊 | `:::grid{columns="1..6" aspect="W/H" fit="cover\|contain"}` 包裹图片 | `image-grid-demo/` |
| 图片尺寸/图注 | `![说明 w-60%](src "图注")` | `[demo removed]` |
| 缩写释义 | `*[SSR]: Server-Side Rendering` 定义行,悬停/聚焦/触屏出释义 | `[demo removed]` |
| 内容标注 | 行内 `[+label]` 引用 + `[+label]:` 定义块,原生 Popover 展示 | `[demo removed]` |
| 行内剧透 | `:spoiler[内容]` | `[demo removed]` |
| GitHub 卡片 | `::github{repo="owner/repo"}`,客户端按需取仓库元数据 | `[demo removed]` |
| 文件包含 | `<!-- @include: 路径 -->`,支持 `{2-6}` 行范围与 `#region` | `[demo removed]` |
| 代码块元数据 | Expressive Code:`title`、`ins={2}`、`del={3-5}`、`collapse={4-8}`、`showLineNumbers`、`frame` 等 | `[demo removed]` |
...
Reference implementation and copyable example: `src/plugins/markdown/manifest.json` and `[demo removed]`.
...
- `src/content/posts/` — 演示文章已移除，语法仍可用
